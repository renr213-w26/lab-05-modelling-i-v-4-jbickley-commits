...
```{r}
births14_habitgiven <- births |> filter(!is.na(habit))
```

```{r}
births14_habitgiven |>
  ggplot(aes(weight, habit))+
  geom_boxplot()+
  labs(x = "Weight (LBS)",title = "Comparison of the Birthweight with Smoking vs Non-Smoking Mothers")
```
We can see that on average, the birth weight of babies who's mothers are smokers in lower, the lightest baby in the set was born to a smoker, and there is a higher IQR as well. In babies with non-smoking mothers the birth weight is on average higher and the heaviest baby was born to a nonsmoker.

```{r}
birth_fit <- linear_reg() |>
  fit(weight ~ habit, data = births14_habitgiven)
tidy(birth_fit)
```
Note: I needed to use ChatGPT to figure out what packages were needed to run this code that is in the notes; as it doesn't say.

The Least Squares Regression Line is $$\hat{Y} = 7.2698731 - 0.592601X$$
...
```{r}
births_mage <- births |> filter(!is.na(mage))
linear_reg() |> fit(weight ~ mage, data =births_mage) |> tidy()
...
The way R determines the line is to find the optimal line that minimizes the sum of square residuals. This makes the estimated regression line the line that is closest as possbile to representing the real data.
...
No, the intercept is not very meaningful because there is no realistic estimate or any sample points where the mother's age is 0 which is what the intercept represents. 

d.  Interpret the slope in the context of the data and the research question.

For every additional year of age of the mother, on average, their babies weight is 0.0142428 LBS heavier. 
...
```{r}
parasites |>
  ggplot(aes(divergence_time,parsim))+
  geom_point()+
  labs(title = "Divergence Time vs Parasite Similarity")
```
...
On average species with higher divergence time have lower parasite similarity. We also see at any given divergence time there is a wide range and likely due to measurements they are clustered around the same divergence time.
...
```{r}
linear_reg() |> fit(parsim ~ divergence_time, data =parasites) |> tidy()
...
The Intercept tells us that with divergence time of 0, the parsim is on average 0.3794 which is not massivley meaningful because there is no time for divergence. The Slope of -0.006531791 tellus us that for every million years there is a a decrease 0f -0.006531 of similarity.
...
It is strange for large divergence times because it is not even close to the data points for the divergence times >60M Years, they are All above the regression line, making it not very accurate for those times.
...
```{r}
parasites |>
  ggplot(aes(divergence_time,transformed_parsim))+
  geom_point()+
  labs(title = "Divergence Time vs Transformed Parasite Similarity")
```
c.  Write a 1-2 sentence description of what you observe in the visualization.

In comparison to the previous section, there are much larger noimal ranges of parasite similarity, however the appear to be more clustered together. It also appears that they move in a more linear fashion, espeically at the higher divergence times. 
...
```
The slopes of the models are the following:
$$\beta_1(Divergence Time) = -0.09066 \\ \beta_1(distance)=0.000274 \\ \beta_1(BMdiff) = -5.044*10^{-6}\\ \beta_1(precdiff) = -0.006013$$
...
There are some strengths and weaknesses, we can see for example that BMdiff is not a useful predictor as there is a wide range of BMdiff values but the parsim is not effected by them. However, all these measures have different units, without looking indepth we cannot really see which one predicts the model and we cannot see which one actually predicts best.
...
```{r}
dt_r2   <- glance(dt_model)$r.squared
dist_r2 <- glance(dist_model)$r.squared
BM_r2   <- glance(BM_model)$r.squared
prec_r2 <- glance(prec_model)$r.squared
...
Yes, this is useful because it actually tells us which one is more correlated to the parsim. We can see that dt_model and dist_model are the best with the other 2 basically being irrelevant. 
...
```{r}
gapminder_07 |> ggplot(aes(gdpPercap,lifeExp)) + 
  geom_point()
gapminder_07 |> ggplot(aes(log(gdpPercap),lifeExp)) + 
  geom_point()
```

It is very clear that the true model of the gdpPercap vs. lifeExp. is not linear, so it is not going to be well fit by a linear model. We should model the log(gdpPerCap) using a linear mode. The gdpPercap vs lifeexp looks like a log function. 
c.  **Model fitting:**

    -   Fit a linear model predicting log gross domestic product from life expectancy. Display the tidy summary.

```{r}
loggdp_model <- linear_reg() |>
  fit(log(gdpPercap) ~ lifeExp, data = gapminder_07)
tidy(loggdp_model)
```
...
```{r}
glance(loggdp_model)$r.squared
lingdp_model <- linear_reg() |>
fit(gdpPercap ~ lifeExp , data = gapminder_07)
glance(lingdp_model)$r.squared

```
A much higher percentage (65.4%) of the life expectancy is explained by loggdp when compared to just gdpPercap (46.1%). 
...
```{r}
gapminder_07 |> ggplot(aes(log(gdpPercap),lifeExp, color = continent)) + 
  geom_point()
```
For the most part the trend appears to be homogeneous between continents, we can see for larger log(gdpPerCap's) that are equated to very similar life capacities regardless of continent. However, there is one clear outlier, Africa where the trand is very different which we can see the number of outlines with large log gdp per capita but relatively small life expectancy. 
...
add_model <- linear_reg() |>
  fit(log(gdpPercap) ~ lifeExp + continent, data = gapminder_07)

tidy(add_model)

```

The intercept of 3.2048  tells us the country with No life expectancy (0 Years) is likely to have a log GDP of 3.2048. 0.0781 tells us that there are is an increase of log GDP of 0.0781 for every year of life expectancy. For each continent we add their slope estimate when the country is in that continent. 

c.  **Prediction:** Predict the GDP of a country in Asia where the average life expectancy is 70 years old.

We do $$3.20484 + 70 * (0.07812444) + 0.01028999$$
```{r}
exp(3.20484 + 70 * (0.07812444) + 0.01028999)
```
So the GDP is $5906.689M Estimate.
## Question 10
...
Generally, a key take-away is that it is very important that you present data in a way that can be easily understood and is visually interesting. The animation is a highly interesting way and likely to encourage people to pay attention to the data. However, some aspects of the presentation are hard to follow as it moves very fast and it can hard to see with that background, it would likely be better if it was not shown in a real space but with solid background; also the scales are somewhat problematic. Furthermore, I believe that they were trying to show too many different things at once to follow.
